...
export class CardService {
  ...
  },
  {
    ...
    negative: "Se na posição vertical, a Morte aponta para um período em que se deixa o passado para trás para que haja espaço para a transformação e a renovação, na posição invertida, este arcano significa que você está resistindo a essa mudança. Relutar contra a morte do passado é tempo perdido. Nada pode contra ela. As mudanças ocorrerão e você tem que aceitar. A resistência só causa dor e sofrimento. Esta carta revela que você ainda carrega crenças prejudiciais do seu passado que estão atrapalhando você a abraçar novas oportunidades que estão surgindo. Por causa da sua resistência, sua vida está estagnada. Você está no limbo. Quando a Morte surge em uma leitura na posição invertida, ela pede para que você abrace a mudança. Veja as possibilidades maravilhosas que estão no horizonte e diga “sim” às transformações.",
